...
## Windows 批处理规范 (重要)
- **中文支持 (强制)：** 所有脚本必须在头部添加 `chcp 65001 >nul` 以确保中文输出不乱码。
- **变量安全：** 在进行 `set /a` 计算或 `if` 比较前，必须初始化变量（如 `set var=0`），并使用引号包裹（如 `if "!var!"=="val"`），防止因 ADB 返回空值导致的语法崩溃。
- **帮助信息 (强制)：** 任何支持参数输入的脚本必须实现 `help` 和 `-h` 参数，清晰列出可用命令及说明。
- **字符转义：** 在 `echo` 中显示 `|`、`&`、`>` 等字符时，必须使用 `^` 进行转义（例如 `echo A ^| B`），否则会被误认为指令。
- **Shell 逻辑 (强制)：** 涉及 Shell 逻辑的操作，优先采用“嵌入式跨平台混合脚本分发模式”，严禁在 Batch 循环内频繁调用 `adb shell`。
