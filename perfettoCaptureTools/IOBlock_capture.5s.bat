...
set devinfo=IOBlock.%model%-%android_version%__%CURR_TIME%
...
echo **************************************************************************
adb shell "echo 0 > /sys/kernel/tracing/tracing_on; echo 1 > /sys/kernel/tracing/events/erofs/enable"
...
adb shell "echo 0 > /sys/kernel/tracing/tracing_on; echo 0 > /sys/kernel/tracing/events/erofs/enable"
echo .
...
(
  ...
  echo  fill_policy: RING_BUFFER
  echo }
  echo duration_ms: 5000
  echo write_into_file: true
  echo file_write_period_ms: 1000
  echo flush_period_ms: 1000
  echo.
  echo data_sources: {
  echo     config {
  ...
  echo             ftrace_events: "ext4/*"
  echo             ftrace_events: "f2fs/*"
  echo             ftrace_events: "erofs/*"
  echo             ftrace_events: "android_fs/*"
