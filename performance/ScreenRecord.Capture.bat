...
if %duration_ms%==10000 (
	set screenRecordtime=10
)else (
	set screenRecordtime=20
)
@REM **************************set record time*********************************
...
set devinfo=ScreenRecord.%model%-%android_version%__%CURR_TIME%
...
for /f "tokens=*" %%A in ('adb shell "dumpsys SurfaceFlinger | grep powermode -i | awk -F'=' '{print $2}'"') do (
    set powerMode=%%A
)
if /i "%powerMode%"=="Off" (
    call :justretry
...
adb shell ^"screenrecord ^-^-time^-limit %screenRecordtime% ^/data^/local^/tmp^/record.mp4 ^& ^
cat ^/data^/local^/tmp^/config.pbtxt ^| perfetto ^-^-txt ^-c ^- ^-o ^/data^/misc^/perfetto-traces^/%traceFile%"
adb shell "echo 0 > /sys/kernel/tracing/tracing_on"
echo .
...
:justretry
(
    echo "Make sure the screen is active before re-running the script"
